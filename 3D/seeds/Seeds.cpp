                    ...
                    const Image<byte>& im2, int i2,int j2,float m2)
{
    ...
    return dist / (sqrt(var1) * sqrt(var2) + EPS);
}
...
static float sum(const Image<byte>& im, int i, int j)
{
    ...
    for (int k=i-win; k<=i+win; k++) {
        for (int l=j-win; l<=j+win; l++) {
            // Warning: Assumes that im(k, l) is well defined!
            s += im(k, l);
        }
    }
    return s;
...
static void find_seeds(Image<byte> im1, Image<byte> im2,
    ...
    for(int y=win; y+win<maxy; y++) {
        ...
        for(int x=win; x+win<im1.width(); x++) {
            int bestD = 0;
            float bestNcc = -1.0f;  // NCC is always bigger than -1.

            for (int d=dMin; d<=dMax; d++) {
                if (x + d < win || x + d > im2.width() - win) {
                    continue;  // As dMax < 0, second condition isn't needed. But let's keep it anyway for a more general case.
                }
            ...
            // If it's above the threshold, it's a seed. Let's add it to the queue and fill the disparity map.
            // One can observe that with no threshold (And therefore no seeds algorithm but a direct dense disparity map)
            // It is much more noisy. The seeds algorithm really improves the results.
            if (bestNcc > nccSeed) {
                seeds(x, y) = true;
                disp(x, y) = bestD;
...
static void propagate(Image<byte> im1, Image<byte> im2,
    ...
    while (! Q.empty()) {
        ...
        for (int i=0; i<4; i++) {
            ...
            if (win<=x && win<=y && x<im2.width()-win && y<im2.height()-win && !seeds(x,y)) {
                int bestD = 0;
                float bestNcc = -1.0f;

                for (int d=max(s.d-1, dMin); d<=min(s.d+1, dMax); d++) {
                    if (x + d < win || x + d > im2.width() - win) {
                        continue;
                    }
                    float ncc = ccorrel(im1, x, y, im2, x + d, y);
                ...
                // Could also go through [s.d-1, s.d+1] and project over [dMin, dMax] afterwards.
                // It gives similar results.
                // bestD = (bestD > dMax)? dMax: bestD;
                // bestD = (bestD < dMin)? dMin: bestD;
                seeds(x, y) = true;
                disp(x, y) = bestD;
                Q.push(Seed(x, y, bestD, bestNcc));
            }
